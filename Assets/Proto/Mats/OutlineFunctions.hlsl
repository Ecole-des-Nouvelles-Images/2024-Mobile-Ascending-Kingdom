...
float4 CalculatePositionHCS(float4 vertex, float3 normal)
{
    VertexPositionInputs vertexInput = GetVertexPositionInputs(vertex.xyz);
    return TransformObjectToHClip(float4(vertex.xyz + normal * _OutlineWidth * 0.1, 1));
}

float4 CalculateFogCoord(float4 vertex)
{
    VertexPositionInputs vertexInput = GetVertexPositionInputs(vertex.xyz);
    return ComputeFogFactor(vertexInput.positionCS.z);
}
